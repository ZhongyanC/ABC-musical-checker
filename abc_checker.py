...
from abc_harmonic_analyzer import HarmonicAnalyzer
        ...
        for vid, (v_idx, idxs) in voices.items():
            ...
            if auto_fix:
                ...
                for line_idx, line in enumerate(lines[idxs[0]:idxs[-1]+1], start=idxs[0]):
                    # 对每一行独立处理，但第一小节规则只能应用到整个声部的第一小节，
                    # 不能在每一行重新触发，否则每行开头的小节都会被误当作弱起跳过。
                    ...
                    if measure_cursor == 0:
                        ...
                        first_expected = exp_voice
                    fixed_line = self._fix_content(
                        line_content, exp_voice, is_cmpd, first_expected
                    )
                    fixed_lines_list.append((line_idx, fixed_line))
                    measure_cursor += nonempty_measure_count
...
"""
    ...
    if os.path.exists(example_path):
        ...
        engine_harm.register_module(HarmonicAnalyzer(verbose=True,chord_voice=True))
        harm_issues, _ = engine_harm.run_pipeline(example_abc, auto_fix=True)
